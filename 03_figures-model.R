...
fit <- readRDS("output/models/fitted_model_LAG7.rds")
end_day <- "2021-06-01"
fit_end <- which(rownames(counts) == end_day) 

# Shapefile for ltlas
shpmap <- st_read("data/processed/geodata/samerica.gpkg") 
shpmap_full <- st_read("data/processed/geodata/samerica.gpkg")
...
# shpmap$reff_mean_AR <- as.numeric(reff_mean[grep("ar.ri", names(reff_mean))])
# shpmap$reff_mean_NE <- as.numeric(reff_mean[grep("ne.ri", names(reff_mean))])
# 
# 
# shpmap_full <- shpmap_full %>% 
#   full_join(st_drop_geometry(shpmap[c("name", "reff_mean_AR", "reff_mean_NE")]))
#   
# reff_mean_AR <- map(x = "reff_mean_AR", shape = shpmap_full, palette = "-RdYlBu", 
#                     breaks = c(0.5, 0.7, 0.8, 0.9, 1, 1.5, 2, 2.5),
#                     digits = 2, legend_title = "Mean relative risk", 
#                     panel_title = "Random Effects - Within country")
#     
# reff_mean_NE <- map(x = "reff_mean_NE", shape = shpmap_full,
#                     palette = "-RdYlBu", n = 7, 
#                     digits = 2, legend_title = "Mean relative risk", 
...
shpmap$within_prop <- within / total 
shpmap$between_prop <- between / total 
shpmap$endemic_prop <- endemic / total 

shpmap_full <- shpmap_full %>% 
  full_join(st_drop_geometry(shpmap[c("name", "within_prop", "between_prop",
                                      "endemic_prop")]))

within <- map(x = "within_prop", shape = shpmap_full, palette = "-RdYlBu",        
                    breaks = seq(0, 1, l = 11),
                    digits = 1, legend_title = "Contribution", 
                    panel_title = "Within cumulative contribution")

between <- map(x = "between_prop", shape = shpmap_full, palette = "-RdYlBu",         
                     breaks = seq(0, 1, l = 11),
                     digits = 1, legend_title = "Contribution", 
                     panel_title = "Between")

endemic <- map(x = "endemic_prop", shape = shpmap_full, palette = "-RdYlBu",         
                     breaks = seq(0, 1, l = 11),
                     digits = 1, legend_title = "Contribution", 
                     panel_title = "Endemic")
...
units_id <- match(units, shpmap$name)
