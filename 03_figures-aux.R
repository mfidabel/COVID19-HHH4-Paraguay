...
# Policy variables
sindex <- readRDS("data/processed/stringency.rds")
testing <- readRDS("data/processed/testing.rds")
vax <- readRDS("data/processed/vax.rds")

units = c("Brazil","Colombia","Paraguay","Peru")
...
all_input %>% 
  select(COUNTRY, time, sindex:vax, rp100k) %>% 
  filter(COUNTRY %in% units) %>% 
  ...
  facet_grid(rows = vars(factor(var, 
                                levels = c("rp100k", "sindex", "testing","vax"),
                                           #"temp", "rain", "sh"),
...
ggplot(all_input, aes(x = time, y = vax)) +
  geom_line() +
  facet_wrap(~ COUNTRY) + 
  labs(x = "", y = "Total vaccinations per 100 people") +
  scale_x_date(date_labels = "%b", expand = c(0, 0)) +
  theme_bw(base_size = 13) 

ggsave("figs/figure_vax.pdf", width = 18, height = 10)
...
samerica$rpk100 <- (samerica$total / samerica$Pop2020) * 100000

rpk <- map(x = "rpk100", shape = samerica, palette = "-RdYlBu",
           n = 10, digits = 0, legend_title = "", 
           panel_title = paste0("Total cases reported per 100,000 up to ", rownames(counts)[dim(counts)[1]]))

tmap_save(rpk, "figs/figure1B.pdf", width = 7, height = 8)

# Map with predictive scores
fit <- readRDS("output/models/fitted_model_LAG7.rds") # final model
...
pv_map <- map(x = "pvalue", shape = samerica, 
              palette = "RdBu", 
...
contrib_ar <- bind_rows(pop_ar, HDI_ar, Sindex_ar, Testing_ar)

contrib_ne <- bind_rows(pop_ne, HDI_ne,Sindex_ne, Testing_ne)
...
panel <- tmap_arrange(pop, HDI, ncol = 2, nrow = 1)
tmap_save(panel, "figs/figureS14.pdf", width = 7 * 2, height = 8 * 2)

ggplot(samerica, aes(x = Age, y = HDI * 100)) +
